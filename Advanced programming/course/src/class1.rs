...
pub fn vals_types(){
    ...
    let mut _last = tuple.2;        // uncomment and see Clion's suggestions
    let mut _last = tuple.0;
    let _last = "ciuao";
    ...
    // let _f = a[6];       // DNC: error: this operation will panic at runtime
...
pub fn expressions(){
    ...
    for (index, element) in a.into_iter().enumerate() {
        println!("Iteration loop: the value is: {} {}", index, element);
    }
