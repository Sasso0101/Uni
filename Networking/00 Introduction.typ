...
#figure(
  image("Images/Protocol stacks.png", width: 50%)
)
=== ISO/OSI structure
+ Physical: takes care of transmitting raw bits on the physical medium (electrical, electro-magnetic, light, sound)
+ Link: handles single hop communication and deals with error discovery and correction
+ Network: delivers packets from one host to another (deals with routing and congestion control)
+ Transport: performs application multiplexing/demultiplexing, segmentation/reassembling of data and deals with quality of service problems
+ Session: setups and maintains a communication between entities, therefore masks network interruptions from the transport layer
+ Presentation: data representation and encoding, encryption
+ Application: provides to applications the means to exchange data
=== Layering
...
#figure(
  image("Images/SAP and protocols.png")
)
==== Data units
...
#figure(
  image("Images/PDU structure.png", width: 70%)
)
Data units can be
