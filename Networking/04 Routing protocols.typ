...
Each link has an associated cost, which is given by the cost function $c: N times N arrow.r NN$. For example for the link $(w, z)$ the associated cost is $c(w, z) = 5$.
...
- $c(x, y)$: link cost from node x to y ($infinity$ if no direct neighbours)
...
#figure(
  image("Images/Dijkstra.png", width: 70%)
)
=== Complexity and problems
In a network of n nodes in the worst case all nodes except the current get checked at every iteration, therefore the time complexity is $(n(n-1))/2=O(n^2)$. Some implementations reduce the complexity to $O("nlog"(n))$.
...
#figure(
  image("Images/Hierarchical OSPF.png", width: 60%)
)
...
#figure(
  image("Images/Distance vector 1.png", width: 60%)
)
...
#figure(
  image("Images/Distance vector 2.png", width: 60%)
)
#figure(
  image("Images/Distance vector 3.png", width: 60%)
)
...
#figure(
  image("Images/Count to infinity 1.png", width: 60%)
)
#figure(
  image("Images/Count to infinity 2.png", width: 60%)
)
