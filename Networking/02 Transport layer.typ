...
#figure(
  image("Images/UDP structure.png", width: 30%)
)
=== Error control
...
#figure(
  image("Images/Stop-And-Wait.png")
)
...
$ d_"trans" = L/R = "8000bit"/(10^9 "bps") = 8 mu s $
...
$ (3L)/(d_"trans"+"RTT") = (24000"bit")/(8mu s+30"ms")=100"kB/s" $
#figure(
  image("Images/Pipelined protocols.png")
)
...
#figure(
  image("Images/Transmission and receive windows.png", width: 60%)
)
...
#figure(
  image("Images/Go-Back-N.png")
)
Note: if the sequence numbers are encoded using k bits, the maximum sender window size has to be $|W_T|<=2^k-1$, because otherwise there could be multiple packets with the same sequence number in the same window.
==== Selective repeat
...
#figure(
  image("Images/Selective repeat.png")
)
In *selective repeat* the maximum window sizes has to be $|W_T|+|W_R|\<=2^k$, so that the receiving window size doesn't wrap around past the sending window size and risk having two packets with the same number in the same window.
#figure(
  image("Images/Window cycle.png", width: 30%)
)
== TCP protocol
...
The number in bytes that the receiver is capable of buffering (receive window) is specified in the RWND (receiver window) field. Assuming that the sender can transmit very quickly, it will have to stop when it sends as many bytes as specified by RWND. Since it is a 16-bit field the maximum data is $2^16=64"kB"$, but we can specify a scaling factor so we can have larger windows.
#figure(
  image("Images/TCP packet structure.png", width: 60%)
)
...
#figure(
  image("Images/Seq and Ack numbers.png", width: 40%)
)
...
#figure(
  image("Images/TCP setup.png", width: 40%)
)
...
#figure(
  image("Images/TCP teardown.png", width: 70%)
)
...
$ "SRTT" = (1-alpha) dot.op "SRTT" + alpha dot.op "RTT" $
...
$ "RTTVAR"=(1-beta) dot.op "RTTVAR" + beta dot.op |"SRTT"-"RTT"| $
...
#figure(
  image("Images/Additive increase multiplicative decrease.png", width: 60%)
)
...
- Congestion avoidance: for every ACK received by transmitter, CWND increased by $"MSS"dot.op "MSS"/"CWND"$. That means that if all packets in a window get ACKed, then the window size increases by one MSS. This induces a linear increase in time.
At the beginning of the transmission CWND gets initialised to 1 and SSTHRESH to RWND. When a timeout happens the algorithm always switches to slow start, retransmits the unACKed packet, SSTHRESH gets set to $max("CWND"/2, 2)$, RTO gets doubled and CWND gets set to 1.
#figure(
  image("Images/Slow start congestion avoidance.png", width: 70%)
)
...
In the fast retransmit and fast recovery algorithm after three duplicated acknowledgements are received the sender retransmits the segment indicated by the duplicated acknowledgements (fast retransmit) and enters fast recovery mode. SSTHRESH gets set to $"CWND"/2$, $W_"LOW"$ still points to the first unACKed segment and CWND gets set to SSTHRESH + 3 MSS. After the sender receives another duplicated acknowledgement CWND gets increased by 1 MSS. After a valid ACK has been received (this means that the lost segment has been successfully received) CWND gets set to SSTHRESH and the sender exits fast recovery and enters congestion avoidance.
#figure(
  image("Images/Fast retransmit fast recovery.png", width: 70%)
)
=== TCP fairness
